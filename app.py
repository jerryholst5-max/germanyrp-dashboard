...
async function loadChannelsAndRoles() {{
  ...
  ['am-channel','am-exempt-ch','lv-channel','ab-panel','ab-log','vs1-notif','vs2-notif','log-channel','tk-panel','tk-transcript','tk-notif','pt-channel','wc-channel','gb-channel','tl-channel','ig-panel','ig-log','bk-channel','rl-channel'].forEach(id => fillSelect(id, channels, '', 'Kanal wahlen...'));
  // Fill voice selects
...
def api_ticketsetup():
    d = request.json; gid = d.get("guild_id"); cfg = get_cfg()
    cfg.setdefault("ticket_setup", {})[gid] = {
        "panel_kanal_id": d.get("panel_kanal_id", ""),
        "transcript_kanal_id": d.get("transcript_kanal_id", ""),
        "notif_kanal_id": d.get("notif_kanal_id", ""),
        "support_role_id": d.get("support_role_id", ""),
        "ping_role_id": d.get("ping_role_id", ""),
        "ticket_category_id": d.get("ticket_category_id", ""),
        "archive_category_id": d.get("archive_category_id", ""),
    }
    save_cfg(cfg); return jsonify({"ok": True})
