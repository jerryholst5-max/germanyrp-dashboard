...
def action_abmeldung_panel():
    ...
    cfg.setdefault("abmeldung_bestaetigung_rolle", {})[gid] = d.get("bestaetigung_rolle_id", "")
    save_cfg(cfg)
    embed = {"title": "📋 Abmeldung", "description": "Klicke auf den Button unten um eine Abmeldung einzureichen.\n\nGib Zeitraum und Grund an — ein Teammitglied wird deine Abmeldung bestätigen.", "color": 0xF0A500, "footer": {"text": "GermanyRP • Abmeldung"}}
    r = discord_send_message(panel_ch, {"embeds": [embed]})
...
def action_ticket_panel():
    ...
    cfg.setdefault("ticket_config", {})[gid] = existing
    save_cfg(cfg)
    embed = {"title": "📩 Support benötigt?", "description": "Klicke auf den Button unten, um ein privates Support-Ticket zu öffnen.\n\nUnser Team wird sich so schnell wie möglich um dein Anliegen kümmern.", "color": 0x82C8F0, "footer": {"text": "GermanyRP • Support"}}
    r = discord_send_message(panel_ch, {"embeds": [embed]})
    if not r.ok: return jsonify({"ok": False, "error": f"Discord: {r.status_code} {r.text}"}), 400
    msg_id = r.json().get("id", "")
    cfg2 = get_cfg()
    cfg2.setdefault("ticket_config", {})[gid]["panel_message_id"] = msg_id
    save_cfg(cfg2)
    return jsonify({"ok": True, "info": "Panel gesendet! Bot-Buttons werden nach Bot-Neustart aktiv."})
...
def action_ingamelog_panel():
    ...
    cfg.setdefault("ingame_log_channel", {})[gid] = d.get("log_kanal_id", "")
    save_cfg(cfg)
    embed = {"title": "🎮 Ingame Log", "description": "Nutze den Button unten um einen Ingame-Vorfall zu melden.\n\nAlle Logs werden im Log-Kanal gespeichert.", "color": 0x82C8F0, "footer": {"text": "GermanyRP • Ingame Log"}}
    r = discord_send_message(panel_ch, {"embeds": [embed]})
    if not r.ok: return jsonify({"ok": False, "error": f"Discord: {r.status_code} {r.text}"}), 400
    return jsonify({"ok": True})
