...
@app.route("/api/partner/public", methods=["GET"])
def api_partner_public():
    """Öffentlicher, ungeschützter Endpunkt für die Website – zeigt nur sichtbare Partner."""
    guild_id = request.args.get("guild_id")
    doc = db["partner_config"].find_one({"guild_id": guild_id}) or {}
    partners = doc.get("partners", [])
    sichtbar = [
        ...
        for p in partners if p.get("sichtbar", True)
    ]
    resp = jsonify(sichtbar)
