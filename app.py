...
async function onGuild(id){if(!id)return;guild=id;const ch=await api('/api/channels?guild_id='+id);const ro=await api('/api/roles?guild_id='+id);const co=ch.map(c=>`<option value="${c.id}">#${c.name}</option>`).join('');const ro2=ro.map(r=>`<option value="${r.id}">@${r.name}</option>`).join('');['log-channel','automod-channel','level-channel','abmeldung-log'].forEach(x=>{const s=document.getElementById(x);if(s)s.innerHTML='<option value="">Kanal wählen...</option>'+co;});['abmeldung-rolle','abmeldung-bestaetigung'].forEach(x=>{const s=document.getElementById(x);if(s)s.innerHTML='<option value="">Rolle wählen...</option>'+ro2;});await loadLogs();await loadAutomod();await loadLevel();await loadPartner();await loadTickets();await loadAbmeldung();}
...
def api_channels():
    ...
    return jsonify([{"id":c["id"],"name":c["name"]} for c in r.json() if c.get("type")==0])
